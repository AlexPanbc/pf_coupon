...
public class SnowFlakeUtil {
    ...
    private final static Logger logger= LoggerFactory.getLogger(SnowFlakeUtil.class);
    ...
    public static SnowFlakeUtil getInstace(){
        try {
            synchronized (SnowFlakeUtil.class) {
                if (null == snowFlakeUtil) {
                    // 模拟在创建对象之前做一些准备工作
                    Thread.sleep(1000);
                    snowFlakeUtil = new SnowFlakeUtil(Constants.MAKE_NUMBER_DATACENTER_ID,Constants.MAKE_NUMBER_MACHINE_ID);
                }
            }
